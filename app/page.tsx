...
const companyRankings = [
  { rank: "01", company: "Synopsys", salary: 255, chill: 5, loading: 1.5, hours: 6, n: 8, tag: "高薪 × 體驗最佳", tone: "lime" },
  { rank: "02", company: "Google", salary: 325, chill: 3, loading: 3, hours: 7.5, n: 8, tag: "薪資領先", tone: "blue" },
  { rank: "03", company: "微軟", salary: 245, chill: 4, loading: 4, hours: 8, n: 7, tag: "高薪 × 高成長", tone: "violet" },
  ...
  return (
    <main>
      ...
      <section className="companies" id="companies"><div className="shell">
        ...
        <div className="company-table" role="table" aria-label="推薦公司薪資比較"><div className="company-row head" role="row"><span>排名／公司</span><span>年薪中位數</span><span>爽度</span><span>Loading</span><span>工時</span><span>樣本</span><span>觀察</span></div>{companyRankings.slice(3).map((c)=><div className="company-row" role="row" key={c.company}><span><i>{c.rank}</i><b>{c.company}</b></span><span><strong>{c.salary}</strong> 萬</span><span>{c.chill} / 5</span><span>{c.loading} / 5</span><span>{c.hours} h</span><span>n = {c.n}</span><span><em>{c.tag}</em></span></div>)}</div>
