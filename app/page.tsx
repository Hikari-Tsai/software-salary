  ...
  return (
    <main>
      ...
      <section className="companies" id="companies"><div className="shell">
        ...
        <div className="company-podium">{companyRankings.slice(0,3).map((c)=><article className={`company-card ${c.tone}`} key={c.company}><div className="company-rank">{c.rank}<span>{c.tag}</span></div><h3>{c.company}</h3><div className="company-salary"><b><AnimatedNumber value={c.salary} /></b><span>萬／年<br />薪資中位數</span></div><div className="company-signals"><span>爽度 <b><AnimatedNumber value={c.chill} /></b>/5</span><span>Loading <b><AnimatedNumber value={c.loading} /></b>/5</span><span>工時 <b><AnimatedNumber value={c.hours} /></b>h</span></div><small>有效樣本 n = {c.n}</small></article>)}</div>
